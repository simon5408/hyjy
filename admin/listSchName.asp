...
<head>
	...
	<script language="javascript" type="text/javascript">
		function doDel(id){
			if(confirm("确定要删除学校信息？")){
				window.location.href="delSchName.asp?schId="+id;
			}
		}
</script>
</head>
...
%>
	...
	<table width="100%" border="0" cellpadding="1" cellspacing="1" style="background:#f5f5f5">
		...
		<tr>
			...
			<td><%=rs("sch_info")%></td>
			<td style="width:80px;">
				<a href='editSchName.asp?schId=<%=rs("sch_id")%>' class="easyui-linkbutton" plain="true">编辑</a>
				<a href='javascript:doDel(<%=rs("sch_id")%>)'class="easyui-linkbutton" plain="true">删除</a>
			</td>
		</tr>
		<% 
			rs.movenext
