...
<head>
	...
	<script language="javascript" type="text/javascript">
		function doDel(id){
			if(confirm("确定要删除教育方式？")){
				window.location.href="delEduType.asp?eduId="+id;
			}
		}
</script>
</head>
...
<% username=session("username") 
	if username<>"" then 
	session("typeName") = "教育方式管理"
%>
<%
	exec="select * from edu_type"
...
%>
	...
	<table width="100%" border="0" cellpadding="1" cellspacing="1" style="background:#f5f5f5">
		...
		<tr>
			<td><%=rs("edu_name")%></td>
			<td style="width:80px;">
				<a href='editEduType.asp?eduId=<%=rs("edu_id")%>' class="easyui-linkbutton" plain="true">编辑</a>
				<a href='javascript:doDel(<%=rs("edu_id")%>)'class="easyui-linkbutton" plain="true">删除</a>
			</td>
		</tr>
		<% 
			rs.movenext
