...
<head>
	...
	<script language="javascript" type="text/javascript">
		function doDel(id){
			if(confirm("确定要删除该图片？")){
				window.location.href="delVipImages.asp?viId="+id;
			}
		}
</script>
</head>
...
<% username=session("username") 
	if username<>"" then 
	session("typeName") = "VIP图片管理"
%>
<%
	exec="select * from vip_img"
...
%>
	...
	<table width="100%" border="0" cellpadding="1" cellspacing="1" style="background:#f5f5f5">
		...
		<tr>
			<td><%=rs("vi_order")%></td>
			<td><img src="vip/<%=rs("vi_path")%>" height="130" width="200"></td>
			<td style="width:80px;">
				<a href='editVipImages.asp?viId=<%=rs("vi_id")%>' class="easyui-linkbutton" plain="true">编辑</a>
				<a href='javascript:doDel(<%=rs("vi_id")%>)'class="easyui-linkbutton" plain="true">删除</a>
			</td>
		</tr>
		<% 
			rs.movenext
